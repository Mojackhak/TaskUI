...
# Build a Windows single-file executable for the Rhythm task.
# Requires: python + PyInstaller (in the chosen environment) and Pillow.

$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$RootDir = Split-Path -Parent (Split-Path -Parent $ScriptDir)
$BuildDir = Join-Path $RootDir "build/rhythm"

$AppName = "Rhythm"
$EntryPoint = Join-Path $RootDir "rhythm/gui/gui.py"
$PngIcon = Join-Path $RootDir "rhythm/icon/icon.png"
...
'@
  ...
  if (Test-Path $PngIcon) {
    $dataArgs = @("--add-data", "$PngIcon;rhythm/icon")
  }
