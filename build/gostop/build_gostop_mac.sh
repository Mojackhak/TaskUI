...
# Build a macOS .app bundle for the Go/No-Go task (GoStop) only.
# Requires: python3 + PyInstaller installed in the current environment.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
BUILD_DIR="$ROOT_DIR/build/gostop"
APP_NAME="GoStop"
ENTRYPOINT="$ROOT_DIR/gostop/gui/gui.py"
PNG_ICON="$ROOT_DIR/gostop/icon/icon.png"
